...
export async function POST(request) {
  ...
  if (!botToken || !chatId) {
    return Response.json({ ok: false, error: "TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID is not configured" }, { status: 500 });
  }
  try {
    ...
    const telegramResponse = await fetch(`https://api.telegram.org/bot${botToken}/sendDocument`, { method: "POST", body: telegramForm });
    const responseText = await telegramResponse.text();
    if (!telegramResponse.ok) {
      return new Response(responseText, { status: telegramResponse.status, headers: { "content-type": "application/json; charset=utf-8" } });
    }
    return new Response(responseText, { status: 200, headers: { "content-type": "application/json; charset=utf-8" } });
  } catch (error) {
